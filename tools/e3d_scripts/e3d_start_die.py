...
#        input("Press key to continue to MomentumSolver")
        ...
        if sys.platform == "win32":
            exitCode = subprocess.call([r"%s" % str(mpiPath), "-n",  "%i" % numProcessors,  "./q2p1_die.exe"])
        else:
            launchCommand = ""

            if paramDict['useSrun']:
                launchCommand = "srun ./q2p1_die"
                if paramDict['singleAngle'] >= 0.0:
                    launchCommand = launchCommand + " -a %d" %(angle)
            else:
                launchCommand = "mpirun -np %i ./q2p1_die" %(numProcessors)
                if paramDict['singleAngle'] >= 0.0 :
                    launchCommand = launchCommand + " -a %d" %(angle)
...
#        input("Press key to continue to HeatSolver")
        ...
        if sys.platform == "win32":
            exitCode = subprocess.call([r"%s" % str(mpiPath), "-n",  "%i" % numProcessors,  "./q2p1_die_temp.exe"])
        else:

            launchCommand = ""

            if paramDict['useSrun']:
                launchCommand = "srun ./q2p1_die_temp"
                if paramDict['singleAngle'] >= 0.0:
                    launchCommand = launchCommand + " -a %d" %(angle)
            else:
                launchCommand = "mpirun -np %i ./q2p1_die_temp" %(numProcessors)
                if paramDict['singleAngle'] >= 0.0 :
                    launchCommand = launchCommand + " -a %d" %(angle)
