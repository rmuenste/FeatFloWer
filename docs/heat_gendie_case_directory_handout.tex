...
started from any current working directory. Neither the input directory nor
the runtime case is restricted to \code{/scratch}; those paths may be anywhere
the user has the required read or write permissions.
...
an input \code{start} directory. On every launch a runtime version is generated at
\code{<case>/start/sampleRigidBody.xml}. This input copy is authoritative and
replaces an older runtime copy.

No OFF files are copied or deleted. Relative geometry references, including
subdirectories, are resolved against the input directory. Absolute references
remain unchanged. The launcher writes absolute segment/sensor paths into the
runtime \code{heat.s3d} and absolute boundary paths into the runtime XML.
Original configurations and geometry remain untouched and may be shared by
multiple runtime cases. All MPI ranks must have access to that input geometry.
Input configurations must be separate from generated runtime configurations.
The existing native readers limit segment paths to 200 bytes and sensor paths
to 255 bytes; those paths cannot contain whitespace or \code{\#}. XML boundary
paths support spaces and are limited to 1023 bytes.
...
runtime \code{heat.s3d} and \code{sampleRigidBody.xml} with absolute references, runs
...
files exist at the absolute paths in the runtime XML.
