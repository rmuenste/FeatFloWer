...
\begin{figure}[hb]
  ...
  \end{center}
  \caption{The CMake configuration GUI}
  \label{fig:cmake-gui}
\end{figure}
...
\subsubsection{Setting Environment Variables for the Python Partitioner}
Copying the Python files in the working directory of the application is not the most efficient workflow and as an additional downside you would have to repeat this process everytime an
important update to the Python partitioner is done. So it is recommended to set environment variables to the location of the Python files in the FeatFloWer source tree, so they get updated
when you fetch and apply new commits from the Git repository. The required environment variables for this to work are called:
\begin{itemize}
