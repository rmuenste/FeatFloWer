...
the required access rights. Your folder structure should look in fig. \ref{fig:folder2} after cloning the mesh repository:
\begin{figure}[hb]
  \begin{center}
...
By installing the partitioner we mean that it is configured in a way that you can simply call it in a terminal, just as you would with any other terminal command. In order to do this, we set environment variables to 
the location of the necessary files in the FeatFloWer source tree, so they get updated when you fetch and apply new commits from the Git repository. The required environment variables for this to work are:
\begin{itemize}
