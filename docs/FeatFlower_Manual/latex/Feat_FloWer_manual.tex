...
\begin{itemize}
  ...
  \item \textbf{PYTHONPATH}: Append the path /path/to/Feat\_FloWer/tools directory to this variable 
\end{itemize}
...
\subsection{Commonly Used CMake Options Overview}
In this section we list some commonly used configure options for the FeatFloWer package. A CMake option MYCMAKE\_OPTION can be set from the command line using the syntax:\\
\cmd{cmake -DMYCMAKE\_OPTION=VALUE [more options] /path/to/Feat\_FloWer} \\

\begin{itemize}
  \item USE\_CGAL:BOOL=True/False : Add the CGAL library, the library will be cloned from a git remote repo. 
  \item USE\_CGAL:BOOL=True USE\_CGAL\_LOCAL:BOOL=True : The combination of these options adds the CGAL library, but uses a local install of the library, in this case set also: 
    \begin{itemize}
      \item CGAL\_INCLUDE\_DIR:PATH : The path to the CGAL include directory 
      \item CGAL\_LIBRARIES:FILEPATH : The path to the needed CGAL libraries, on unix-like system these are usually called libCGAL.a and libCGAL\_Core.a 
    \end{itemize}
  \item Q2P1\_BUILD\_ID:STRING : Set a build-id (effectively sets compiler, compile flags, build type)
\end{itemize}

\subsection*{Examples}
Setting CGAL libraries: 
\begin{itemize}
  \item -DCGAL\_LIBRARIES="/path/libCGAL.a;/path/libCGAL\_Core.a" 
\end{itemize}
Settings CGAL include directory:
\begin{itemize}
  \item -DCGAL\_INCLUDE\_DIR="/path/cgal/include" 
\end{itemize}
