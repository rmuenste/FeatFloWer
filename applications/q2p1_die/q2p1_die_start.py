...
"""
A python launcher script for the FeatFloWer DIE application
"""
import sys
import getopt
import platform
...
def usage():
  print("Usage: python q2p1_die_start.py [options]")
  print("Where options can be:")
  print("[-h, --help]: prints this message")
  print("[-f, --case-folder]: Configuration folder for the particular DIE case")
...
def main():
  """ This function is the entry point of the driver script. It first checks the
      ...
      Simulation
      =====================
      The DIE simulation is split up in several parallel simulations that run in a sequence.
      ...
      Meshing
      =====================
      The DIE application will use the mesh folder under the path <_data/meshDir>.
  ...
  for i in range(nmin,nmax):  # nmax means the loop goes to nmax-1
    ...
    if sys.platform == "win32":
      shutil.copyfile("Release/q2p1_die.exe", "./q2p1_die.exe")
      print("Computing with angle: %f" % angle)
      print("Command:mpiexec -n %i ./q2p1_die.exe" % numProcessors)
      subprocess.call(["mpiexec", "-n", str(numProcessors), "./q2p1_die.exe"])
      iangle = int(angle)
      shutil.copyfile("_data/prot.txt", "_data/prot_%04d.txt" % iangle)
    else:
      subprocess.call(['mpirun -np %i ./q2p1_die' % numProcessors],shell=True)
      iangle = int(angle)
      shutil.copyfile("_data/prot.txt", "_data/prot_%04d.txt" % iangle)
