...
SUBROUTINE General_init_ext(MDATA,MFILE)
 ...
 USE var_QuadScalar, ONLY : cGridFileName,nSubCoarseMesh,cProjectFile,&
   ...
   bAttractionSmootherEnable,bUmbrellaDistanceWeightEnable,cAttractionDistanceProvider
 USE Transport_Q2P1, ONLY : Init_FAC3D_Handlers, Init_QuadScalar,LinSc,QuadSc
 USE mesh_smoothing_hooks, ONLY : ApplyAttractionSmootherHook
 USE attraction_smoother, ONLY : AttractionSmoother_ComputeDistance, &
                                 AttractionSmoother_ComputeWeight
 ...
 bAttractionSmootherEnable = .TRUE.
 bUmbrellaDistanceWeightEnable = .TRUE.
 cAttractionDistanceProvider = "cylinder"
...
CALL ApplyAttractionSmootherHook(mg_mesh, nlmax)
...
IF (myid.NE.0) THEN
  ...
  CALL AttractionSmoother_ComputeDistance(mg_mesh%level(mg_mesh%maxlevel)%nvt, &
    mg_mesh%level(mg_mesh%maxlevel)%dcorvg, dCylDist)
  CALL AttractionSmoother_ComputeWeight(mg_mesh%level(mg_mesh%maxlevel)%nvt, &
    dCylDist, dCylWeight)
