...
do i=1,nvt
 if (mg_mesh%level(ilev)%knpr(i).ne.1) then
  CALL RANDOM_NUMBER(dNMB)
  if (dNMB.GT.0.9d0) mg_mesh%level(ilev)%knpr(i) = 1
 end if
end do
...
! ----------------------------------------------
SUBROUTINE FillUpParticleElement(RF,E,KNPR,bV,iIEL)
integer iIEL
type(RefinerMesh) RF
logical bV(8)
real*8 E(3,8)
...
real*8 :: xi1,xi2,xi3,DJAC(3,3),DETJ
real*8 , allocatable ::cBP(:,:)
integer i,j,k,ii,MyEl,nnvt,nnel
real*8 :: dPhi = 0.0d0,x
character*256 cF
integer, allocatable :: kvert(:,:)
logical :: bRefine

!  if (myTemplate.ne.0)  cF = 'Particle/ParticleX.tri'
!  if (myTemplate.eq.0)  cF = 'Particle/ParticleB20.tri'
!  if (myTemplate.eq.8)  cF = 'Particle/ParticleB24.tri'
!  if (myTemplate.eq.18) cF = 'Particle/ParticleB26.tri'
!  if (myTemplate.eq.21) cF = 'Particle/ParticleB27.tri'
! else
 if (myTemplate.ne.0)  cF = 'Particle/ParticleX.tri'
 if (myTemplate.eq.0)  cF = 'Particle/Particle20.tri'
 if (myTemplate.eq.8)  cF = 'Particle/Particle24.tri'
 if (myTemplate.eq.18) cF = 'Particle/Particle26.tri'
 if (myTemplate.eq.21) cF = 'Particle/Particle27.tri'
! end if
...
allocate(xRF%dcoor(3,xRF%nOfVert))
allocate(xRF%knpr(xRF%nOfVert))
...
if (bV(MyEl)) then
 write(cF,'(A,I1,A)') 'Particle/Patch_E',MyEl,'.tri'
else
 write(cF,'(A,I1,A)') 'Particle/Patch_Y',MyEl,'.tri'
!  write(cF,'(A,I1,A)') 'Particle/Patch_X',MyEl,'.tri'
end if
...
DO i=1,nnel
 read(1,*) xRF%kvert(:,inel+i)
end do
DO i=1,nnel
 xRF%kvert(:,inel+i) = xRF%kvert(:,inel+i) + invt
end do
 
read(1,*)

DO i=1,nnvt
 read(1,*) xRF%knpr(invt+i)
end do
...
CALL CleanupSmallPatch(xRF,RF,E)

! RF%dcoor = xRF%dcoor
! RF%kvert = xRF%kvert
! RF%knpr  = xRF%knpr
DEallocate(xRF%dcoor)
DEallocate(xRF%kvert)
DEallocate(xRF%knpr)

deallocate(cBP)

END SUBROUTINE FillUpSubParticleElement
