subroutine init_q1_scalar(log_unit)
  ...
  USE Transport_Q2P1, ONLY : Init_QuadScalar_Structures_sse, &
    ProlongateSolution,InitCond_Velocity_PF
  USE ViscoScalar, ONLY : Init_ViscoScalar_Stuctures, &
    Transport_ViscoScalar,IniProf_ViscoScalar,ProlongateViscoSolution
  USE Transport_Q1, ONLY : GetInterfacePoints_ALPHA_PF_Q1, &
    Init_GenLinSc_ALPHA_Q1,ProlongateSolution_GenLinSc_Q1
  ...
  IF (ISTART.EQ.0) THEN
    ...
    call Load_ListFiles_General(0,'v,x,p,s,q')
    CALL GetInterfacePoints_ALPHA_PF_Q1(log_unit,0)
    CALL InitCond_Velocity_PF()
  ELSE
    IF (ISTART.EQ.1) THEN
      IF (myid.ne.0) CALL CreateDumpStructures(1)
      call Load_ListFiles_General(0,'v,x,p,s,q,d')
      CALL GetInterfacePoints_ALPHA_PF_Q1(log_unit,0)
      call InitCond_Velocity_PF()
    ELSE
...
END SUBROUTINE General_init_ext
 ...
 SUBROUTINE myGDATNEW (cName,iCurrentStatus)
   ...
   USE var_QuadScalar, ONLY : myMatrixRenewal,bNonNewtonian,cGridFileName,&
     ...
     bViscoElastic,bRefFrame,nInitUmbrellaSteps
