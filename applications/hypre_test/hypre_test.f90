...
!     Set the rhs values to h^2 and the solution to zero
      do i = 1, local_size
         ...
         rows(i) = ilower + i -1
!          if (myid.eq.0) then
!            write(*,*) "myid:",myid,"index:",i,"; rows:", ilower + i -1-1
!           end if
      enddo
...
!     AMG
      if ( solver_id .eq. 0 ) then
         ...
         write(*,*) "IS SOLVER ALLOCATED?", solver
!        Create solver
         call HYPRE_BoomerAMGCreate(solver, ierr)

         write(*,*) "IS SOLVER NOW ALLOCATED?", solver
...
!                   6 Falgout coarsening (uses 1 rst, followed by CLJP using the interior coarse points
!                   generated by 1 as its rst independent set)
!                   7 CLJP-coarsening (using a xed random vector, for debugging purposes only)
...
!                   9 PMIS-coarsening (using a xed random vector, for debugging purposes only)
!                   10 HMIS-coarsening (uses one pass Ruge-Stueben on each processor independently, followed
!                   by PMIS using the interior C-points generated as its rst independent set)
...
!                     8 `1-scaled hybrid symmetric Gauss-Seidel
!                     9 Gaussian elimination (only on coarsest level)
!                     13 `1 Gauss-Seidel, forward solve
!                     14 `1 Gauss-Seidel, backward solve
!                     15 CG (warning - not a xed smoother - may require FGMRES)
!                     16 Chebyshev
!                     17 FCF-Jacobi
!                     18 `1-scaled jacobi
!          call HYPRE_BoomerAMGSetRelaxType(solver, 16, ierr)
!        RelaxType
!                     0 the points are relaxed in natural or lexicographic order on each processor
!                     1 CF-relaxation is used, i.e on the ne grid and the down cycle the coarse points are relaxed rst,
!                     followed by the ne points; on the up cycle the F-points are relaxed rst, followed by the C-points.
...
!        Sweeeps on each level
         call HYPRE_BoomerAMGSetNumSweeps(solver, 10, ierr)
!        Interpolation type
!                     0 classical modied interpolation
!                     1 LS interpolation (for use with GSMG)
!                     2 classical modied interpolation for hyperbolic PDEs
...
!        Trunc Factor// Does not work?
!                     Defines a truncation factor for the interpolation. The default is 0.
!          call HYPRE_BoomerAMGSetTruncFactor(solver, 0, ierr)
!        Max numbers per rows
!                   Optional) Denes the maximal number of elements per row for the interpolation. The default is 4. To turn
!                   of truncation, it needs to be set to 0.
         call HYPRE_BoomerAMGSetPMaxElmts(solver, 7, ierr)
!        aggressive coarsening
!                     (Optional) Denes the number of levels of aggressive coarsening. The default is 0, i.e. no aggressive coars-
!                     ening.
!          call HYPRE_BoomerAMGSetAggNumLevels(solver, 0, ierr)
...
! !                     4 multipass interpolation
!          call HYPRE_BoomerAMGSetAggInterpType(solver, 2, ierr)
...
!        ----------------------------------------------------------
         ...
         call HYPRE_BoomerAMGSetSmoothNumSwps(solver, 5, ierr)
         call HYPRE_BoomerAMGSetDropTol(solver, 0.01, ierr)
!        ----------------------------------------------------------
...
!         call HYPRE_ParCSRGMRESGetNumIterations(solver, num_iterations,ierr)
!         call HYPRE_ParCSRGMRESGetFinalRelative(solver, final_res_norm,ierr)
