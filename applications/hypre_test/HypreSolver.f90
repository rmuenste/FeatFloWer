...
!        Set the values for row i
     ...
     call HYPRE_IJMatrixSetValues(&
         A, 1, tmp(1), tmp(2), cols, values, ierr)
     !$omp end target data
  enddo
...
!     Set the rhs values to h^2 and the solution to zero
  ...
  call HYPRE_IJVectorSetValues(&
      b, local_size, rows, rhs_values, ierr)
  call HYPRE_IJVectorSetValues(&
      x, local_size, rows, x_values, ierr)
  !$omp end target data
...
!        Now setup and solve!
     call HYPRE_BoomerAMGSetup(&
         solver, parcsr_A, par_b, par_x, ierr)
