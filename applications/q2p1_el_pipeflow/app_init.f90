...
SUBROUTINE General_init_ext(MDATA,MFILE)
 ...
 ! q2p1_el_pipeflow: leave dPeriodicity at its default (1d9 => effectively off),
 ! mirroring q2p1_el_frozen_trace. Forcing it to the unit-box length matches
 ! opposite wall DOFs as periodic partners during PARENTCOMM, which builds
 ! inconsistent neighbour structures on the pre-partitioned mesh and corrupts
 ! E013_CreateComm. The one-way terminal-velocity case (u_f == 0) needs no
 ! periodic coupling, so the wall-bounded box is the correct setup here.
...
IF (myid.eq.1) write(*,*) 'done!'
 ...
 if (myid .ne. 0) then
   ...
   ! decomposition follows the CFD mesh partition axis, and creates one mobile
   ! sphere with no obstacle cylinder / walls (unlike commf2c_el_frozen_trace).
   ...
   IF (bOK) THEN
     ...
     IF (TRIM(ADJUSTL(cVar)).EQ.TRIM(ADJUSTL(cName))) THEN
       ...
       CASE ("Periodic")
         ...
         ! myGDATNEW runs before the partition read in General_init_ext, and the
       ...
       CASE ("ELInertialLift")
         READ(string(iEq+1:),*) el_inertial_lift
       CASE ("ELInertialLiftUmax")
