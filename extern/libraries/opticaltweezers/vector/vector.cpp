...
Vector<complex<double> > *conv2vector (int Anz, double *r)
{
 ...
 int j=1;
 Erg=new Vector<complex<double> >[Anz];
 for (int i=0; i<Anz/3; i++)
 {
...
{
 Vector<double> Erg;
 Erg[0]=sqrt(x*x+y*y+z*z);
 if (z!=0.0) Erg[1]=atan2(sqrt(x*x+y*y),z); else Erg[1]=0.0;
 if (x!=0.0) Erg[2]=atan2(y,x); else Erg[2]=0.0;
...
{
 cout << endl;
 cout << "Fehler bei Vektorrechnung: " << S << endl;
 exit(1);
}
