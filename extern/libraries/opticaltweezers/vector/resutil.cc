...
{
 os << parms.nx << "  " << parms.ny << "   " << parms.alpha << "  " << parms.db << endl;
 os << parms.dx << "  " << parms.dy << "   " << parms.dxy   << "  " << parms.n0 << endl;
 os << parms.bmax << "  " << parms.r0 << "   " << parms.r0end   << "  " << parms.l0 << endl;
 os << parms.k0 << "  " << parms.pol << "   " << parms.ResRad << "   " << parms.ResAzi << endl;
 os << parms.AnzEin << "  " << parms.AngleTol << "  " << parms.evan << endl;
 os << parms.phase << "  " << parms.logscale << "  " << parms.tunneln << "  " << parms.ColMax << "  " << parms.ColMin 
    << "  " << parms.EinX << endl;
 os << parms.AnzReflex << endl;   
 return os;
}
...
istream& operator >> (istream& is, GlobalParms& parms)
{
 ...
 //is >> parms.AnzEin   >> parms.AngleTol   >> parms.evan >> endl;
 is >> parms.phase   >> parms.logscale   >> parms.tunneln   >> parms.ColMax   >> parms.ColMin;
 is >> parms.EinX;
 return is;
}

double absc (const complex<double>& z) 
{
 ...
 for (double Py=-Parms.bmax;  Py<=Parms.bmax; Py+=dPy)
  {
   ...
   if (Parms.pol==SENKRECHT) Strahl[iS].S.EAmp=Vector<double>(0,0,1); 
   else Strahl[iS].S.EAmp=Vector<double>(0,1,0);
   Strahl[iS].S.E=1.0;
   Strahl[iS].S.k=Vector<double>(1,0,0);
   //dreh(Strahl[iS].S.k,Parms.alpha);
   //dreh(Strahl[iS].S.EAmp,Parms.alpha);
   Strahl[iS].S.phi=0.0;
   Strahl[iS].P=Vector<double>(-Parms.bmax,Py,0);
   iS++;
  }
}
...
void clear (GlobalParms parms, Vector<complex<double> > **Gitter)
{
 for (int j=0; j<parms.ny; j++)
   ...
   {
    Gitter[j][k]=Vector<complex<double> >(0.0,0.0,0.0);
   } 
}
...
Vector <complex<double> > **newGitter (int n, int m)
{
 Vector <complex<double> > **Erg;
 Erg=(Vector<complex<double> > **) calloc (n,sizeof(Vector<complex<double> >*));
 for (int i=0; i<=n; i++)
  Erg[i]=(Vector<complex<double> >*) calloc (m,sizeof(Vector<complex<double> >));
 for (int i=0; i<n+1; i++)
   for (int j=0; j<m+1; j++)
   Erg[i][j]=Vector<complex<double> > (0.0,0.0,0.0); 
 return Erg;
}
...
/*
  ...
  AnzEin  : Anzahl der Einschluesse 
...

{
 ...
 for (int i=0; i<AnzEin; i++)
 {
  B=S.k*(anf-Ein[i].P*r0);
  C=abs(anf-Ein[i].P*r0);
  C=C*C-Ein[i].a*Ein[i].a*r0*r0;
  D=B*B-C;
  if (D>=0.0)
  {
   l1=sqrt(D);
                               ...
                               const  Vector<double>& k)
{
    ...
    sucht den n‰chsten Gitterpunkt, wenn P0 nicht (sicher) auf einer 
    Gitterstrebe sitzt
    
    Wird von <next> verwendet.
    Parameter:
    parms : Infos ueber die Kugel (auﬂen)
...
*/
Vector<double> next (const GlobalParms &parms,const Vector<double> &P0, 
                     const Vector<double> &k)
{
...
*/
 ...
 if ((x1-x0)*(x1-x0)+(y1-y0)*(y1-y0)<(x2-x0)*(x2-x0)+(y2-y0)*(y2-y0))  
     Erg=Vector<double> (x1*parms.dx-parms.r0,y1*parms.dy-parms.r0,0.0); 
 else 
     Erg=Vector<double> (x2*parms.dx-parms.r0,y2*parms.dy-parms.r0,0.0);   
 return Erg;
}
...
{
 int i=int (x/dx);
 if (fabs((double)(i+1)*dx-x) <= 1.0E-10) i++;
...
/*
  Berechnet den n‰chsten Schnittpunkt eines Strahls der Richtung k mit einer Kugel, die sich am Ort OK
  befindet und einen Radius rK hat. P ist der Ort an dem man sich gerade befindet
*/ 
{
 Vector<double> Ps,Erg;
 double l1,l2,l;
 double P2,Pk,k2;
 Ps=P-OK;
 k2=k*k;
 ...
 if (l1<0.0) { found=false; return Vector<double> (INF,INF,INF);}
 l2=(-Pk+sqrt(l1))/k2;
 l1=(-Pk-sqrt(l1))/k2;
 if (l1<=1E-10) l=l2; else l=l1;
 if (l<=0.0) { found=false; return Vector<double> (INF,INF,INF);}
 Erg=P+l*k;
 found=true;
 return Erg;
}
...
{ 
 ...
 os.write ((char *) &parms.AnzEin,(char) sizeof(parms.AnzEin));
...
istream & loadbinGlobalParms (istream &is, GlobalParms &parms)
{ 
 is.read ((char *) &parms.nx,(char) sizeof(parms.nx));
 cout << "parms.nx=" << parms.nx << endl;
 ...
 is.read ((char *) &parms.AnzEin,(char) sizeof(parms.AnzEin));
...
GlobalParms readGlobalParms (bool old , ifstream &is)
{
 ...
 is.read ((char *) &erg.AnzEin, (char) sizeof (erg.AnzEin));
