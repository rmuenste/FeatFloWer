...
FormEllipsoid::FormEllipsoid ()
: Form()
{
 type = ELLIPSOID;
}

FormEllipsoid::FormEllipsoid (const Form &F) : Form (F)
{
 type = ELLIPSOID;
}

FormEllipsoid::FormEllipsoid (const FormEllipsoid &E)
:Form (E)
{
 r=E.r;
 P=E.P;
...
: Form (P, n, alpha, Ex, Ey, Ez,ELLIPSOID)
{
 Vector<double> h[8];
 trafo (Ex,Ey,Ez,H,R);
 this->r0=r0;
 ...
 this->P=P;
 type=ELLIPSOID;
 P2=Vector<double> (P[0]*P[0],P[1]*P[1],P[2]*P[2]);
 r_2=Vector<double> (1.0/(r[0]*r[0]),1.0/(r[1]*r[1]),1.0/(r[2]*r[2]));
  initQuad();
}

void FormEllipsoid::scale(double sf)
{
...
bool FormEllipsoid::next(const Vector<double> &Ps, const Vector<double> &K,
                     Vector<double> &pout,const int inside)
{
 bool innen;
 double A,B,C;
 double l1,l2,l;
 Vector<double> n,k,p=Ps-P;
 p=H*p; 
 k=H*K;
 pout = Ps;
 n=norm(Ps); 
/* cout.precision (10);
 ...
 Vector<double> k2=Vector<double> (k[0]*k[0],k[1]*k[1],k[2]*k[2]);
 Vector<double> p2=Vector<double> (p[0]*p[0],p[1]*p[1],p[2]*p[2]);
 A=k2*r_2;
 B=2.0*emult(p,k)*r_2;
 C=p2*r_2-1.0;
...
Vector<double> FormEllipsoid::norm (const Vector<double> &p)
{
 Vector<double> ps,n;
 ps=H*(p-P);
 
 n=Vector<double> (2.0*ps[0]/(r[0]*r[0]),2.0*ps[1]/(r[1]*r[1]),2.0*ps[2]/(r[2]*r[2]));  
 n=R*n/abs(n);
 return n;
}
...
/* Ellipsoid & Ellipsoid::operator = (const Ellipsoid& e)
 ...
 bool FormEllipsoid::isInside (const Vector<double> &p)
 {
  Vector<double> h;
  h=ediv (p,r);
  return abs2(h)<1.0;
 ...
 FormEllipsoid *FormEllipsoid::copy ()
 {
   return new FormEllipsoid (P,r,n,r0,alpha,e[0],e[1],e[2]);
 }

 void FormEllipsoid::copy(FormEllipsoid *E)
 {
   E->H=H;
   E->R=R;
   E->P=P;
...
FormEllipsoid & FormEllipsoid::operator = (FormEllipsoid &f)
{
...
void FormEllipsoid::setr0(double r0)
{
 Vector<double> h[8];
 r=r/this->r0*r0;
 r_2=Vector<double> (1/(r[0]*r[0]),1/(r[1]*r[1]),1/(r[2]*r[2]));
 P=P/this->r0*r0;
// por=Vector<double> (r[0],r[1],r[2]);
// pul=-por;
// por=R*por+P;
// pul=R*pul+P;
 initQuad();
...
void FormEllipsoid::binWrite (ofstream &os)
{
 P.binWrite(os);
 H.binWrite(os);
 R.binWrite(os);
...
void FormEllipsoid::binRead (ifstream &is)
{
 type=ELLIPSOID;
 P.binRead(is);
 H.binRead(is);
...
ostream &operator << (ostream &os,FormEllipsoid E)
{
 os << "%r=" << E.r << endl;
 os << "%P=" << E.P << endl;
 os << "%Ealpha=" << E.Ealpha/M_PI*180.0
    << "°   Ebeta=" << E.Ebeta/M_PI*180.0
    << "°   Egamma=" << E.Egamma/M_PI*180.0 << "°" << endl;
 return os;
}

double FormEllipsoid::Volume ()
{
 return 4.0/3.0*M_PI*r[0]*r[1]*r[2];
}

void FormEllipsoid::setr (Vector<double> &r) 
  { 
    this->r=r; 
    r_2=Vector<double> (1.0/(r[0]*r[0]),1.0/(r[1]*r[1]),1.0/(r[2]*r[2])); 
  }
  void FormEllipsoid::setr (double a,double b, double c) 
   { 
    r=Vector<double> (a,b,c); 
    r_2=Vector<double> (1.0/(r[0]*r[0]),1.0/(r[1]*r[1]),1.0/(r[2]*r[2])); 
   }
 
 void FormEllipsoid::seta(double a, bool VConst) 
 { 
  Vector<double> h;  
  a=a*r0;
  if (VConst) 
  {
   double t=sqrt(r[0]/a);
   h=Vector<double>(a,r[1]*t,r[2]*t);  
  }
  else h=Vector<double>(a,r[1],r[2]); 
  setr(h); 
 }

 void FormEllipsoid::setb(double b, bool VConst) 
 { 
  Vector<double> h;  
  b=b*r0;
  if (VConst) 
  {
   double t=sqrt(r[1]/b);
   h=Vector<double>(r[0]*t,b,r[2]*t);  
  }
  else h=Vector<double>(r[0],b,r[2]); 
  setr(h); 
 }

 void FormEllipsoid::setc(double c, bool VConst) 
 { 
  Vector<double> h;  
  c=c*r0;
  if (VConst) 
  {
   double t=sqrt(r[2]/c);
   h=Vector<double>(r[0]*t,r[1]*t,c);  
  }
  else h=Vector<double>(r[0],r[1],c); 
  setr(h); 
 }
