...
!      end if
    DO J=1,nSIZE
     IF (((ABS(mg_mpi(ILEV)%parST(pID)%CoragLinkX(1,I)-mg_mpi(ILEV)%parST(pID)%CoragLinkX(2,J)).LT.DEpsPrec)).AND.&
         ((ABS(mg_mpi(ILEV)%parST(pID)%CoragLinkY(1,I)-mg_mpi(ILEV)%parST(pID)%CoragLinkY(2,J)).LT.DEpsPrec)).AND.&
         ((ABS(mg_mpi(ILEV)%parST(pID)%CoragLinkZ(1,I)-mg_mpi(ILEV)%parST(pID)%CoragLinkZ(2,J)).LT.DEpsPrec))) THEN
...
LOGICAL DDATA(*)
INTEGER N,SRC
INTEGER ITAG,STATUS(3),I
!REAL*8, ALLOCATABLE :: RBUFF(:)

ITAG=7

!ALLOCATE (RBUFF(N))
CALL MPI_RECV(DDATA,N,MPI_LOGICAL,SRC,ITAG,MPI_COMM_WORLD,STATUS,IERR)
!DO I=1,N
!DDATA(I)=RBUFF(I)
!END DO
!DEALLOCATE (RBUFF)
...
!ALLOCATE (RBUFF(N))
CALL MPI_RECV(DDATA,N,MPI_DOUBLE_PRECISION,SRC,ITAG,MPI_COMM_WORLD,STATUS,IERR)
!DO I=1,N
!DDATA(I)=RBUFF(I)
!END DO
!DEALLOCATE (RBUFF)
...
INTEGER N,SRC
INTEGER ITAG,STATUS(3),I
REAL*4, ALLOCATABLE :: RBUFF(:)

ITAG=3

!ALLOCATE (RBUFF(N))
CALL MPI_RECV(DDATA,N,MPI_REAL,SRC,ITAG,MPI_COMM_WORLD,STATUS,IERR)
!DO I=1,N
!DDATA(I)=RBUFF(I)
!END DO
!DEALLOCATE (RBUFF)
...
!ALLOCATE (RBUFF(N))
CALL MPI_RECV(DDATA,N,MPI_INTEGER,SRC,ITAG,MPI_COMM_WORLD,STATUS,IERR)
!DO I=1,N
!DDATA(I)=RBUFF(I)
!END DO
!DEALLOCATE (RBUFF)
...
CALL MPI_RECV(DDATA,1,MPI_INTEGER,SRC,ITAG,MPI_COMM_WORLD,STATUS,IERR)
...
CALL MPI_RECV(RBUFF,1,MPI_DOUBLE_PRECISION,SRC,ITAG,MPI_COMM_WORLD,STATUS,IERR)
DDATA=RBUFF
