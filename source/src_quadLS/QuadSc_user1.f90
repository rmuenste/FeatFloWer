...
dist1 = sqrt(x*x + (y-myCenterY)*(y-myCenterY)) - myFish%radius
...
IF (t_i.le.myFish%t_open) THEN
 dAlpha =  0.583d0*(t_i/myFish%t_open)*PI + 0.055d0*PI
ELSE
 dAlpha = -0.583d0*((t_i-myFish%t_open)/(myFish%period-myFish%t_open))*PI+ 0.638d0*PI
END IF
...
ELSE
 ...
 IF (t_i.le.myFish%t_open) THEN
  ValU = +0.1821d0*PI*(Y-(myFish%myCenterY+myFish%centerRotDisp(2)))
  ValV = -0.1821d0*PI*(X-myFish%centerRotDisp(1)) + myFish%myVelY
 ELSE
  ValU = -0.728d0*PI*(Y-(myFish%myCenterY+myFish%centerRotDisp(2)))
  ValV = +0.728d0*PI*(X-myFish%centerRotDisp(1)) + myFish%myVelY
 END IF
