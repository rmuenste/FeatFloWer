...
!
!************************************************************************
      subroutine GetForcesTime(U1,U2,U3,P,ALPHA,DVISC,KVERT,KAREA,KEDGE,&
                           DCORVG,ELE)
!************************************************************************
!*     Discrete convection operator: Q1~ elements (nonparametric)
!*-----------------------------------------------------------------------
...
!
      ...
      REAL*8 Center(3),dForce(6)
!     
      real*8, dimension(:), allocatable :: myForce
      COMMON /OUTPUT/ M,MT,MKEYB,MTERM,MERR,MPROT,MSYS,MTRC,IRECL8
      COMMON /ERRCTL/ IER,ICHECK
      COMMON /CHAR/   SUB,FMT(3),CPARAM
      COMMON /ELEM/   DX(NNVE),DY(NNVE),DZ(NNVE),DJAC(3,3),DETJ,&
...
!
      SAVE
!
      ALLOCATE(myForce(6*myFBM%nParticles))
      IF (myid.eq.0) GOTO 999
!
      DO 1 I= 1,NNDER
1     BDER(I)=.FALSE.
...
!
      iPointer = 6*(IP-1)
      myForce(iPointer+1) = DResForceX
      myForce(iPointer+2) = DResForceY
      myForce(iPointer+3) = DResForceZ
      myForce(iPointer+4) = DTrqForceX
      myForce(iPointer+5) = DTrqForceY
      myForce(iPointer+6) = DTrqForceZ

      END DO ! nParticles

999   CALL COMM_SUMMN(myForce,6*myFBM%nParticles)
      ...
      END DO
       ...
       !WRITE(*,*) 'myid: ',myid,' number of elements: ',nnel
