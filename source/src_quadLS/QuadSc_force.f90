...
!************************************************************************
      SUBROUTINE GetForcesCyl(U1,U2,U3,P,bALPHA,DVISC,KVERT,KAREA,KEDGE,&
                              DCORVG,DResForce,ELE)
!************************************************************************
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)

      REAL*8  U1(*),U2(*),U3(*),P(*),DVISC(*),DCORVG(3,*)
      REAL*8  DResForce(3)
      LOGICAL bALPHA(*)
      INTEGER KVERT(8,*),KAREA(6,*),KEDGE(12,*)
      EXTERNAL ELE

      CALL GetForceCyl(U1,U2,U3,P,bALPHA,DVISC,KVERT,KAREA,KEDGE, &
                       DCORVG,DResForce,ELE)
...
!************************************************************************
      SUBROUTINE GetForceCyl(U1,U2,U3,P,bALPHA,DVISC,KVERT,KAREA,KEDGE,&
                             DCORVG,DResForce,ELE)
!************************************************************************
!*     FBM alpha-based force integration for a single body.
!*     Free-form equivalent of the legacy routine in QuadSc_force.f.
!*-----------------------------------------------------------------------
      USE PP3D_MPI, ONLY:myid,showID,COMM_SUMMN
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
      CHARACTER SUB*6,FMT*15,CPARAM*120
...
!
        ...
        DO I=1,IDFL
          IG=KDFG(I)
          IF (bALPHA(IG)) NJALFA=NJALFA+1
          IF (.NOT.bALPHA(IG)) NIALFA=NIALFA+1
        END DO
...
!
! The integrand is:
!     f = sigma_h . (-grad(alpha)) = -p (-grad(alpha)) + mu (grad(u) + grad(u)^T) . (-grad(alpha))
!
! Symmetric stress tensor: sigma_h = -p I + mu (grad(u) + grad(u)^T)
!
          DN1=-DALX
          DN2=-DALY
          DN3=-DALZ
...
!************************************************************************
      SUBROUTINE GetForcesCylPerCell(U1,U2,U3,P,bALPHA,DVISC,KVERT,&
                    KAREA,KEDGE,DCORVG,DCellForce,DCellNormal,&
                    DCellVol,DCellSymGrad,NEL_in,ELE)
!************************************************************************
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)

      REAL*8  U1(*),U2(*),U3(*),P(*),DVISC(*),DCORVG(3,*)
      REAL*8  DCellForce(3,NEL_in),DCellNormal(3,NEL_in)
      REAL*8  DCellSymGrad(3,NEL_in)
      REAL*8  DCellVol(NEL_in)
      LOGICAL bALPHA(*)
      INTEGER KVERT(8,*),KAREA(6,*),KEDGE(12,*)
      INTEGER NEL_in
      EXTERNAL ELE

      CALL GetForceCylPerCell(U1,U2,U3,P,bALPHA,DVISC,KVERT,KAREA,&
                    KEDGE,DCORVG,DCellForce,DCellNormal,DCellVol,&
                    DCellSymGrad,NEL_in,ELE)

      END

!************************************************************************
      SUBROUTINE GetForceCylPerCell(U1,U2,U3,P,bALPHA,DVISC,KVERT,&
                    KAREA,KEDGE,DCORVG,DCellForce,DCellNormal,&
                    DCellVol,DCellSymGrad,NEL_in,ELE)
!************************************************************************
!*     FBM alpha-based force integration for a single body.
!*     Per-cell variant: stores force, normal, volume and
!*     symmetric velocity gradient per element.
!*-----------------------------------------------------------------------
...
!
      DO I=1,NEL_in
        ...
        DCellNormal(3,I) = 0D0
        DCellVol(I) = 0D0
        DCellSymGrad(1,I) = 0D0
        DCellSymGrad(2,I) = 0D0
        DCellSymGrad(3,I) = 0D0
      END DO
!
! *** Loop over all elements
      DO IEL=1,NEL
...
!
          ...
          AH3=-Press*DN3 + DNY*((DU3X+DU1Z)*DN1 + (DU3Y+DU2Z)*DN2 &
              + (DU3Z+DU3Z)*DN3)
!
          DCellForce(1,IEL) = DCellForce(1,IEL) + AH1*OM
          DCellForce(2,IEL) = DCellForce(2,IEL) + AH2*OM
          DCellForce(3,IEL) = DCellForce(3,IEL) + AH3*OM
!
          DCellNormal(1,IEL) = DCellNormal(1,IEL) + DN1*OM
          DCellNormal(2,IEL) = DCellNormal(2,IEL) + DN2*OM
          DCellNormal(3,IEL) = DCellNormal(3,IEL) + DN3*OM
!
          DCellVol(IEL) = DCellVol(IEL) + OM
!
! n . (grad(u) + grad(u)^T) — symmetric velocity gradient contracted with normal
          DCellSymGrad(1,IEL) = DCellSymGrad(1,IEL) + &
              ((DU1X+DU1X)*DN1 + (DU1Y+DU2X)*DN2 &
              + (DU1Z+DU3X)*DN3)*OM
          DCellSymGrad(2,IEL) = DCellSymGrad(2,IEL) + &
              ((DU2X+DU1Y)*DN1 + (DU2Y+DU2Y)*DN2 &
              + (DU2Z+DU3Y)*DN3)*OM
          DCellSymGrad(3,IEL) = DCellSymGrad(3,IEL) + &
              ((DU3X+DU1Z)*DN1 + (DU3Y+DU2Z)*DN2 &
              + (DU3Z+DU3Z)*DN3)*OM
!
        END DO
!
