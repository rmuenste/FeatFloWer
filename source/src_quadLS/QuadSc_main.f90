...
! ALLOCATE (myALE%MeshVelo(3,mydof))
 ...
 IF (myid.eq.1) write(*,*) 'Quad checking parallel structures for Q2  on level : ',ILEV
...
!    if(FictKnpr(i) .ne. FictKNPR_uint64(i)%bytes(1) + 1)then
!      write(*,*) myid,') critical error at dof= ',i, ' fk=',FictKnpr(i), ' uint=',FictKNPR_uint64(i)%bytes(1) + 1
!    else
!      write(*,*) myid,') ok at dof= ',i
!    end if
!  else
!    if(FictKnpr(i) .ne. FictKNPR_uint64(i)%bytes(1) + 1)then
!      write(*,*) myid,') critical error at dof= ',i, ' fk=',FictKnpr(i), ' uint=',FictKNPR_uint64(i)%bytes(1) + 1
...
! ----------------------------------------------
!
SUBROUTINE updateFBMGeometry(silent)
use cinterface, only: calculateFBM
...
if(present(silent)) isSilent = silent
 ...
 if (calculateFBM()) then
  ...
  CALL QuadScalar_FictKnpr(mg_mesh%level(ilev)%dcorvg,&
    ...
    mg_mesh%level(ilev)%kvert,&
    mg_mesh%level(ilev)%kedge,&
    mg_mesh%level(ilev)%karea, isSilent)
...
! ----------------------------------------------
!
SUBROUTINE SET_FBMVAL_QuadScalar()
  use fbm, only: fbm_velBCUpdate
  implicit none
  REAL*8 PX,PY,PZ,DAUX, sum
  INTEGER i,inpr,minpr,inprU,inprV,inprW,ndof,iType, itotal
  real*8 p27(3,27),x27(27),v27(3,27)
  integer f27(27),i27(27)
  real*8 p8(3,8),x8(8),v8(3,8)
  integer f8(8),i8(8)
  real*8 :: ParticleRadius=0.4d0
  real*8 q(3),w(3),dist
  ...
  if (.not.allocated(FBMWeight)) allocate(FBMWeight(ndof))
  if (.not.allocated(FBMVelocity)) allocate(FBMVelocity(3,ndof))
  FBMWeight = 0d0
  FBMVelocity = 0d0

  DO i=1,ndof
    IF (QuadSc%knprU(ilev)%x(i).EQ.1) FBMVelocity(1,i)=0d0
    IF (QuadSc%knprV(ilev)%x(i).EQ.1) FBMVelocity(2,i)=0d0
    IF (QuadSc%knprW(ilev)%x(i).EQ.1) FBMVelocity(3,i)=0d0
    
    IF (QuadSc%knprU(ilev)%x(i).EQ.1.and.&
        QuadSc%knprV(ilev)%x(i).EQ.1.and.&
        QuadSc%knprW(ilev)%x(i).EQ.1) FBMWeight(i) = 1d0
    
    IF (myBoundary%iInflow(i).NE.0) THEN 
      PX = myQ2Coor(1,i);  PY = myQ2Coor(2,i);  PZ = myQ2Coor(3,i)
      CALL GetVeloBCVal(PX,PY,PZ,FBMVelocity(1,i),FBMVelocity(2,i),FBMVelocity(3,i),myBoundary%iInflow(i),timens)
      FBMWeight(i) = 1d0
    END IF
  end do
...
!   DO i=1,ndof
!    PX = myQ2Coor(1,i);  PY = myQ2Coor(2,i);  PZ = myQ2Coor(3,i)
!    CALL fbm_FindClosestParticle(PX,PY,PZ,Q(1),Q(2),Q(3),W(1),W(2),W(3))
!    dist = SQRT((Q(1)-PX)**2d0 + (Q(2)-PY)**2d0 + (Q(3)-PZ)**2d0) - ParticleRadius
!    daux = 1d0-max(0d0,min(1d0,dist/0.25d0))
!    if (daux.gt.FBMWeight(i)) then
!     FBMWeight(i) = daux
!     FBMVelocity(:,i) = W
!    end if
!   end do
  ...
  do iel=1,nel
   CALL NDFGL(iel,1,11,mg_mesh%level(ilev)%kvert,mg_mesh%level(ilev)%kedge,mg_mesh%level(ilev)%karea,iKDFG,iKDFL)
   ...
   do i=1,8
    ...
    I8(idfl)   = idfg
    P8(:,idfl) = mg_mesh%level(ilev)%dcorvg(:,idfg)
    F8(idfl)   = FictKNPR(idfg)
    X8(idfl)   = FBMWeight(idfg)
    V8(:,idfl) = FBMVelocity(:,idfg)
   end do
   
   CALL CheckFBMElem8(P8,F8,I8,X8,V8,iel)
   
   FBMWeight(I8(:)) = x8(:)
   FBMVelocity(1,I8(:)) = v8(1,:)
   FBMVelocity(2,I8(:)) = v8(2,:)
   FBMVelocity(3,I8(:)) = v8(3,:)
...
!     
!     I27(idfl)   = idfg
!     P27(:,idfl) = mg_mesh%level(ilev)%dcorvg(:,idfg)
!     F27(idfl)   = FictKNPR(idfg)
!     X27(idfl)   = FBMWeight(idfg)
!     V27(:,idfl) = FBMVelocity(:,idfg)
!    end do
!    
!    CALL CheckFBMElem27(P27,F27,I27,X27,V27,iel)
!    
!    FBMWeight(I27(:)) = x27(:)
!    FBMVelocity(1,I27(:)) = v27(1,:)
!    FBMVelocity(2,I27(:)) = v27(2,:)
!    FBMVelocity(3,I27(:)) = v27(3,:)
!    
!   end do

END SUBROUTINE SET_FBMVAL_QuadScalar
!
! ----------------------------------------------
!
