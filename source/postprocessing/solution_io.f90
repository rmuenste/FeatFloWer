...
! @param filehandle Unit of the output file
!
subroutine postprocessing_sse_temp(dout, inlU,inlT,filehandle)
  ...
  IF (itns.eq.1) THEN
    ...
    call viz_output_fields_Simple(myExport, int(myProcess%angle), QuadSc, LinSc, & !Tracer, &
                           Viscosity, Screw, Shell, Shearrate,&
                           mg_mesh)
...
!     CALL ZTIME(myStat%t1)
  ...
  IF(dout.LE.(timens+1e-10)) THEN
    ...
    IF (itns.ne.1) THEN
      CALL ZTIME(myStat%t0)
      iXgmv = int(myProcess%angle)
      call viz_output_fields_Simple(myExport, iXgmv, QuadSc, LinSc, & !Tracer, &
                             Viscosity, Screw, Shell, Shearrate, mg_mesh)
...
end subroutine postprocessing_sse_temp
!
!================================================================================================
! Wrapper routine for writing the solution to a file 
!================================================================================================
