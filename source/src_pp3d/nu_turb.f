      SUBROUTINE GET_NuTURB(U1,U2,U3,DSTRS,KVERT,KAREA,KEDGE,
     *                      KINT,DCORVG,ISIDE,CONST,DELT,DNUT)
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
C
      PARAMETER (NNBAS=27,NNDER=10,NNCUBP=36,NNVE=8,NNEE=12,NNAE=6,
     *           NNDIM=3,NNCOF=10)
C
      DIMENSION U1(*),U2(*),U3(*),KINT(*)
      DIMENSION DSTRS(*),DNUT(*),DELT(*),ISIDE(*)
...
C
      SAVE 
C
      IF (IELT.EQ.0) CALL DER_UVW_P(U1,U2,U3,DSTRS,
     *    KVERT,KAREA,KEDGE,KINT,DCORVG,ISIDE,E031)
      IF (IELT.EQ.1) CALL DER_UVW_P(U1,U2,U3,DSTRS,
     *    KVERT,KAREA,KEDGE,KINT,DCORVG,ISIDE,E030)
      IF (IELT.EQ.2) CALL DER_UVW_N(U1,U2,U3,DSTRS,
     *    KVERT,KAREA,KEDGE,KINT,DCORVG,ISIDE,EM31)
      IF (IELT.EQ.3) CALL DER_UVW_N(U1,U2,U3,DSTRS,
     *    KVERT,KAREA,KEDGE,KINT,DCORVG,ISIDE,EM30)
C
      DO 2 IAT=1,NAT
       DNUT(IAT) = ((CONST*DELT(IAT))**2)*DSQRT(DSTRS(IAT))
2     CONTINUE
C
      END
C
...
C
************************************************************************
      SUBROUTINE DER_UVW_P(U1,U2,U3,DSTRSG,KVERT,KAREA,KEDGE,KINT,
     *                     DCORVG,ISIDE,ELE)
************************************************************************
*     Deformation tensor, PARAMETRIC VERSION
*-----------------------------------------------------------------------
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
...
C
      DIMENSION U1(*),U2(*),U3(*),KINT(*),ISIDE(*)
      DIMENSION DSTRSG(*),DSTRSL(NNAE)
      DIMENSION DCORVG(3,*),KVERT(NNVE,*),KAREA(NNAE,*),KEDGE(NNEE,*)
      DIMENSION KDFG(NNBAS),KDFL(NNBAS),GRADU(NNDIM,NNDIM)
      DIMENSION DU(NNDIM,NNAE)
C
...
C
      ...
      IDFL=NDFL(IELTYP)
C
      ICUB=2                          !! OMierka -> Midpoints of areas !!
      CALL CB3H(ICUB)
...
220   CONTINUE
C
      D_S = 2D0*(GRADU(1,1)*GRADU(1,1)                         + !OMierka
     *           GRADU(2,2)*GRADU(2,2)                         + !OMierka
     *           GRADU(3,3)*GRADU(3,3)                        )+ !OMierka
     *          (GRADU(1,2)+GRADU(2,1))*(GRADU(1,2)+GRADU(2,1))+ !OMierka
     *          (GRADU(1,3)+GRADU(3,1))*(GRADU(1,3)+GRADU(3,1))+ !OMierka
     *          (GRADU(2,3)+GRADU(3,2))*(GRADU(2,3)+GRADU(3,2))  !OMierka
C
      DO 230 JDOFE=1,IDFL
       JDFL=KDFL(JDOFE)
       IF (DBAS(1,JDFL,1).GT.0.3D0) THEN
        DSTRSL(JDOFE)=D_S                                        ! OMierka
        GOTO 235
       END IF
230   CONTINUE
C
C      WRITE(*,*) "The criterion for nu_turb has failed"
C      STOP
235   CONTINUE
200   CONTINUE
C
      DO 300 JDOFE=1,IDFL
       JDFG=KDFG(JDOFE)
       ISIDE(JDFG)  = ISIDE(JDFG) + 1
       IF (ISIDE(JDFG).EQ.1) THEN
        DSTRSG(JDFG) =                      1.0D0*DSTRSL(JDOFE)
       ELSE
        DSTRSG(JDFG) = 0.5D0*DSTRSG(JDFG) + 0.5D0*DSTRSL(JDOFE)
       END IF
300   CONTINUE
C
100   CONTINUE
...
C
************************************************************************
      SUBROUTINE DER_UVW_N(U1,U2,U3,DSTRSG,KVERT,KAREA,KEDGE,KINT,
     *                     DCORVG,ISIDE,ELE)
************************************************************************
*     Deformation tensor, NONPARAMETRIC VERSION
*-----------------------------------------------------------------------
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
      CHARACTER SUB*6,FMT*15,CPARAM*120
C
...
C
      DO 230 JDOFE=1,IDFL
       JDFL=KDFL(JDOFE)
       IF (DBAS(1,JDFL,1).GT.0.6D0) THEN
        DSTRSL(JDOFE)=D_S                                        ! OMierka
        GOTO 235
       END IF
230   CONTINUE
