      ...
      PARAMETER (NNWORK=160000000,NNARR=299)
...
!       CALL FBM_FromFile(cFBM_File)
      ...
      ELSE
       IF (ISTART.EQ.1) THEN
        IF (myid.ne.0) CALL CreateDumpStructures(1)
!        CALL FBM_FromFile(cFBM_File)
        CALL LoadExtremeSmartDumpSolution(CSTART,1)
!        CALL Load_DUMPProfiles(CSTART)
       ELSE
        IF (myid.ne.0) CALL CreateDumpStructures(0)
!        CALL FBM_FromFile(cFBM_File)
        CALL LoadExtremeSmartDumpSolution(CSTART,0)
       ...
       IF(tout.LE.(timens+1e-10)) THEN
        ...
        IF (insav.NE.0.AND.itns.NE.1) THEN
         IF (MOD(iOGMV,insav).EQ.0) THEN
          CALL ZTIME(myStat%t0)
          CALL ReleaseExtremeSmartDumpSolution(-1)
