************************************************************************
      SUBROUTINE RI_CONVDG(U1,U2,U3,DLS,DA,NA,KCOLA,KLDA,KVERT,KAREA, 
     *                  KEDGE,KINT,DCORVG,DEPS,ELE)
************************************************************************
*     Discrete convection operator: Q1~ elements (parametric)
*-----------------------------------------------------------------------
...
!          DDLS = (DLS(JDFG)-0d0)
!          DFAC = DDLS/(DDLS*DDLS+DEPS)
!          DNSS = DFAC*DABS(DDLS)
         DNSS = DLS(JDFG)

         ! Normalized normal direction
         DNDD = DSQRT(U1(JDFG)*U1(JDFG)+
     *   U2(JDFG)*U2(JDFG) + U3(JDFG)*U3(JDFG))

         DNXX = DNSS*U1(JDFG)/DNDD
         DNYY = DNSS*U2(JDFG)/DNDD
         DNZZ = DNSS*U3(JDFG)/DNDD
...
C
************************************************************************
      SUBROUTINE RI_CONVNP(U1,U2,U3,DLS,DA,NA,KCOLA,KLDA,KVERT,KAREA,
     *                  KEDGE,KINT,DCORVG,DEPS,ELE)
************************************************************************
...
C
************************************************************************
      SUBROUTINE RI_MASSNP(DLS,DD,NA,KCOLA,KLDA,KVERT,KAREA,
     *                     KEDGE,KINT,DCORVG,DTIME,DEPS,ELE)
************************************************************************
*     Discrete convection operator: Q1~ elements (nonparametric)
*-----------------------------------------------------------------------
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
...
!         DDLS = (DLS(JDFG)-0d0)
!         DFAC = DDLS/(DDLS*DDLS+DEPS)
        DNSS = DNSS + DLS(JDFG)*HBAS
!         DNSS = DNSS + DFAC*DABS(DDLS)*HBAS
210   CONTINUE
C
      DO 230 JDFL=1,IDFL
        IG=KDFG(JDFL)
        HBAS=DBAS(1,KDFL(JDFL),1)
        DD(IG)=DD(IG) + OM*DNSS*HBAS*DTIME
230   CONTINUE
C
200   CONTINUE
C
...
C
************************************************************************
      SUBROUTINE LS_CORRNP(DLS,DVOL,DSURF,DCORR,
     *        KVERT,KAREA,KEDGE,KINT,DCORVG,AVOL,ELE)
************************************************************************
*     Discrete convection operator: Q1~ elements (nonparametric)
*-----------------------------------------------------------------------
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
...
!     -------------------------------------------
!     Find elements which form the interphase or 
!     are inside the secondary phase
!     -------------------------------------------
      NJALFA=0
      NIALFA=0
      DO IAT=1,6
        ...
        ENDIF
      ENDDO
      IF(NIALFA.EQ.6) GOTO 100
      IF(NJALFA.EQ.6) THEN
       DVOL = DVOL + DBLE(AVOL(IEL))
       GOTO 100
      END IF
!     -------------------------------------------
C
...
C
       DV =0D0     ! ALFA
       DO I=1,IDFL
         DV = DV + DLS(KDFG(I))*DBAS(1,KDFL(I),1)
       ENDDO
C
       IF (DV.LE.0d0) DDV=1d0
       IF (DV.GT.0d0) DDV=0d0
