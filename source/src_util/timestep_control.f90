MODULE timestep_control
  ...
  PRIVATE
  PUBLIC :: SetSimulationTimeStep, GetSimulationTimeStep
...
CONTAINS
  ...
  SUBROUTINE GetSimulationTimeStep(dTimeStep)
    IMPLICIT NONE

    REAL*8, INTENT(OUT) :: dTimeStep
...
#ifdef HAVE_PE
    INTERFACE
      SUBROUTINE get_pe_timestep(dTime) BIND(C, name="get_pe_timestep_")
        USE, INTRINSIC :: iso_c_binding, ONLY: c_double
        REAL(c_double), INTENT(OUT) :: dTime
      END SUBROUTINE get_pe_timestep
    END INTERFACE
#endif

#ifdef HAVE_PE
    CALL get_pe_timestep(dTimeStep)
#else
    dTimeStep = TSTEP
#endif
  END SUBROUTINE GetSimulationTimeStep
