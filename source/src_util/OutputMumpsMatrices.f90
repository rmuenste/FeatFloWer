...
SUBROUTINE OUTPUT_BurgersSOL_MASTER(N,NZ,IRN,JCN,RHS)
...
write(cF,'(A,I2.2,A,I2.2,A)')  'input_',subnodes,'/SOL_',MYID,'.txt'
open(file=adjustl(trim(cF)),unit=myFile, action="write",iostat=istat)
! WRITE('(A)') "'"//adjustl(trim(cF))//"'"

WRITE(myFile,'(I0)') N

DO i=1,N
 WRITE(myFile,'(ES16.8)') RHS(i)
END DO
...
write(cF,'(A,I2.2,A,I2.2,A)')  'input_',subnodes,'/RHS_',MYID,'.txt'
open(file=adjustl(trim(cF)),unit=myFile, action="write",iostat=istat)
! WRITE('(A)') "'"//adjustl(trim(cF))// "'"
...
DO i=1,NZ_loc
 WRITE(myFile,'(I0,A,I0,A,ES16.8)') IRN_loc(i),' ',JCN_loc(i),' ',A_loc(i)
END DO
