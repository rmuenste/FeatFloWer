...
Core solvers live in `source/`, with submodules such as `src_pp3d` (Q2/P1 Navier–Stokes), `src_visco` (non-Newtonian rheology), and `src_fbm` (fictitious boundary coupling). Executable front-ends are under `applications/` and follow the `q2p1_*` naming scheme. The `libs/` and `extern/` trees host third-party components (PE, contact dynamics, CGAL helpers). Canonical parameter files reside in `_data/`, while benchmark meshes and setups are collected in `testcases/`. Start with `docs/md_docs/README.md` to find implementation references, subsystem notes, and runbooks. For a complete from-scratch workflow to configure, compile, partition, and run a basic benchmark application, see `docs/md_docs/guide_01_q2p1_fc_ext_cylinder_benchmark_from_scratch.md`. When a task mentions `q2p1_sse`, `q2p1_sse_temp`, SSE/TSE/DIE/gendie extrusion workflows, `setup.e3d`, `Extrud3D.dat`, or `e3d_start.py`/`e3d_start_yaml.py`, first consult `docs/md_docs/q2p1_sse_index.md`; it links focused notes on launch/staging, YAML simulation plans, initialization, configuration/state, geometry, rheology, solver loop, temperature extension, and output/finalization. When a task mentions the standalone `heat` application, EWIKON heat workflows, `_data/heat.s3d`, `heat_start.py`, heat sensors, heat-object geometry, or heat convergence/regulation, first consult `docs/md_docs/heat_index.md`; it links focused notes on launch/staging, initialization, parsing/state, geometry handling, main solver steps, postprocessing, and convergence evaluation. Build recipes are also maintained in `build_guide.md`.
...
- Before modifying `libs/pe` or the FeatFloWer-PE interface, read `libs/pe/AGENTS.md` and follow its PE-specific documentation and workflow guidance.
